    ...
    <ul class="block lg:flex">
        @foreach($categories as $category)
            @if ($category->children->count())
                <li class="group">
                    @if ($currentCategoryName === $category->slug || $category->children->pluck('slug')->contains($currentCategoryName))
                    ...
                    @endif
                        <ul class="dropdown-navigation-submenu absolute hidden group-hover:block bg-white shadow-lg">
                            @foreach($category->children as $subCategory)
                                    <li><a class="block py-2 px-4 {{$currentCategoryName === $subCategory->slug ? 'text-orange' : 'text-black'}} hover:text-orange hover:bg-gray-100" href="{{route('catalog.index', $subCategory)}}">{{$subCategory->name}}</a></li>
                            @endforeach
                        </ul>
                </li>   
            @else
                <li class="group"><a class="inline-block p-4 {{$currentCategoryName === $category->slug ? 'text-orange' : 'text-black'}}  font-bold hover:text-orange" href="{{route('catalog.index', $category)}}">{{$category->name}}</a></li>
            @endif
        @endforeach
    </ul>
 </nav>
